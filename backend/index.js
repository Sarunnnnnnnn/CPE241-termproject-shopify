...
app.post("/addProduct", (req, res) => {
  ...
  if (jwtStatus !== false) {
    ...
    const unitweight = req.body.weight;

    const sql = 'INSERT INTO products(shop_id, product_name, product_description, category, sub_category, product_unitprice, product_unitweight) VALUES ((SELECT shop_id FROM shops WHERE owner_id  = ?), ?, ?, ?, ?, ?, ?) ';
    const values = [owner_id, fullName, description, category, subcategory, unitprice, unitweight];
...
app.post("/editProfile", (req, res) => {
  ...
  if (jwtStatus !== false) {
    ...
    const sql = 'UPDATE customers SET customer_firstname = ?, customer_lastname = ?, customer_bod = ?, 	customer_gender = ?, customer_email = ?, customer_tel_no = ? WHERE customer_id = ?';
    const values = [firstName, lastName, dateOfBirth, gender, email, phoneNumber, customer_id];
...
app.post("/get_order",(req,res)=>{
  ...
  if(jwtStatus!=false){
    const owner_id = jwtStatus.id;
    const sql = 'SELECT order_items.quantity, order_items.price, orders.paid_at, shipping_addresses.*, products.product_name, products.image ,shops.shop_id FROM order_items LEFT JOIN orders ON order_items.order_id = orders.order_id LEFT JOIN shipping_addresses ON orders.shipping_address_id = shipping_addresses.shipping_address_id LEFT JOIN products ON order_items.product_id = products.product_id LEFT JOIN shops ON products.shop_id = shops.shop_id WHERE shops.owner_id = ?';
    const values = [owner_id];
      connection.query(sql, values, (err, results, fields) => {
        if (err) {
          console.log('Error querying database:', err);
