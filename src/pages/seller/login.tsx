...
const LoginSeller = ({ onSubmit }: LoginFormProps) => {
  ...
  return (
    <div className=" m-4  bg-white  max-w-md mx-auto p-4  ">
      <h1 className="text-3xl font-bold m-4 p-4 mb-8 text-[#48466D] ml-[250px] ">Login Seller</h1>
      <form onSubmit={handleSubmit} className='grid grid-cols-2 gap-4 '>
        ...
        <br />
        <button type="submit" className='justify-end m-4 w-24 rounded-md border bg-[#48466D] px-5 py-2 text-sm font-medium text-white hover:bg-[#605d91] transition duration-300 ml-[40px] '>Sign in</button>
      </form>
      {loginError && <div className="text-red-500 ml-[250px]">{loginError}</div>}
      <Link to="/signupSeller" className=" font-poppins">
      <button type="submit" className='justify-end m-1 w-21  rounded-md border bg-[#7773b8] px-5 py-2 text-[12px] font-medium text-white hover:bg-[#605d91] transition duration-300 ml-[260px] '>Sign up</button>
      </Link>
